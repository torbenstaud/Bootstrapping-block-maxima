...
load(here("scripts/mle/fixedR/bst/data/CiFreRfix"))
ciFreTibRfix
#create tibbles----

#create factors----
ciFreTibRFixPlot <- ciFreTibRfix %>% filter(
  beta %in% c(0, 0.5)
)
...
ciFreTibRFixPlot$k <- factor(ciFreTibRFixPlot$k, levels = c(2,3,0,1),
...
ciFreTibRFixPlot$marginal <- factor(ciFreTibRFixPlot$marginal,
                                    levels = c(3,4),
                                    labels = c("Fréchet", "Pareto")
)

ciFreTibRFixPlot$beta <- factor(ciFreTibRFixPlot$beta, 
                                levels = c(0, 0.25, 0.5, 0.75),
                                labels = c(parse(text = TeX("i.i.d.")), 
                                           parse(text = TeX("ARMAX $0.25$")), 
                                           parse(text = TeX("ARMAX $0.5$")), 
...
ciFreTibRFixPlot$alpha <- factor(ciFreTibRFixPlot$alpha, 
                                 levels = c(0.5, 1, 1.5), 
                                 labels = c(parse(text = TeX("$\\alpha_0 = 0.5$")),
                                            parse(text = TeX("$\\alpha_0 = 1$")),
                                            parse(text = TeX("$\\alpha_0 = 1.5$")))
...
ciWidthDb <- ciFreTibRFixPlot %>% filter(k == "db") %>% select(-c(k,empCov)) %>% 
  rename(dbWidth = "avgWidth")
ciWidthRel <- left_join(ciFreTibRFixPlot, ciWidthDb) %>% 
...
if(FALSE){
  ggsave(combCiPlot, path = here("results/"), 
         filename = "plotFreFixRCi.pdf", device = "pdf",
         width = 10, height = 10, 
  )
}
