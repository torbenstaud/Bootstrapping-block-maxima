...
library(patchwork)
#source files----
sourcePath <- here(file.path("backend", "src"))
dataPath <- here(file.path("backend", "data", "7", "7.2"))
section <- "7.Case study/7.2 Precipitation data from a weather station"

source(here(sourcePath, "0source.R"))
source(here(sourcePath, "0source_ts.R"))
sourceCpp(here(sourcePath, "0sourceCn.cpp"))
source(here(sourcePath, "sourceCase.R"))
textSize <- 20
...
fileName <- "produkt_klima_tag_17810101_20241231_02290.txt" #Hohenpeißenberg
dataRaw <- unz(file.path(dataPath, "Hohenpeißenberg.zip"), fileName) %>% 
...
if(F){#17mins with B = 10**3
  ...
  cat(format(tDel))
  save(DbArr, CbArr, file = file.path(dataPath, "array_rl_cs-0.30.5"))
}
load(file = file.path(dataPath, "array_rl_cs-0.30.5")) # DbArr, CbArr
...
load(file.path(dataPath, "coeff_rl0.5"))
...
rm(tmpTib)
ciTibPlt <- avgCiTib %>% filter(type != "sb")
...
if(F){#17mins with B = 10**3
  ...
  tDel <- difftime(Sys.time(), t0)
  cat(format(tDel))
  save(DbArrMean, CbArrMean, file.path(dataPath, "array_mean_prcp_cs"))
}
load(file = file.path(dataPath, "array_mean_prcp_cs"))
# create tibble with data
ciTibMean <- bind_rows(
  tibble(
    year = yearVec[seq(1,nY - winSize + 1)],
...
##corecct the boundaries
load(file.path(dataPath, "coeff_rl"))
...
plotRlCsPrcp <- 
  ...
  #  Faceting via `parameter` (Columns) & `panel` (Row)
...
if(T){
  ggsave(plot = plot_combinedCsFaCoRlMean, path = file.path(here(section), "plots"), 
         filename = "Figure 11.pdf",
         device = "pdf", width = 13, height = 7) 
}
