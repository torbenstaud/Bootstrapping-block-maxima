...
## for positive constrained
gev_mle_cppPos_lvec <- function(data, start = NULL, method = "L-BFGS-B") {
  # Prüfe Eingabedaten
  if (!is.numeric(data) || any(!is.finite(data))) {
    stop("Die Eingabedaten müssen numerisch und endlich sein.")
  }
  ...
  #  "LBFGSB" damit wir nur positive shapes anschauen
  resultBfgs <- try({
    optim(
      par = start,
      fn = function(theta) neg_log_likelihood_gev_lvec(theta, data),
      method = "L-BFGS-B",
      lower = c(-10, 0.01, 10**(-6)),
      upper = c(50, 50, 0.5),
      control = list(maxit = 10**5, fnscale = 10**3) # ohne fnscale probieren, fnscale = sum(data))
    )
  }, silent = TRUE)
  ...
  # Versuch mit "DEoptim"
  cat("Fehler bei 'L-BFGS-B'. Versuche 'BFGS'.\n")
  result <- try({
    optim(
      par = start,
      fn = function(theta) neg_log_likelihood_gev_lvec(theta, data),
...
gev_mle_cppPos <- function(data, start = NULL){
  if (!is.numeric(data) || any(!is.finite(data))) {
    stop("Die Eingabedaten müssen numerisch und endlich sein.")
  }
...
ciCircmaxRlPos <- 
  function(xx, Time = 100, r, k, niv = 0.05, B = 10^3, topcutoffGam = 0.5, 
    ...
    if(mthd == "cb"){
      ...
      for(indB in seq(1,B)){
        while(!(theta_bst[3] <= topcutoffGam & theta_bst[3] >= botcutoffGam)){
          ...
          theta_bst <- gev_mle_cppPos_lvec(bsts)$par
        }
      ...
      for(indB in seq(1,B)){
        while(!(theta_bst[3] <= topcutoffGam & theta_bst[3] >= botcutoffGam)){
          ...
          bsts <- xxMb[indBst]
          theta_bst <- gev_mle_cppPos(bsts)$par
    ...
    #cat(format(tDe l))
    theta_bst <- gev_mle_cppPos(xxMb)$par
    est_bst <- gev_rl(Time, theta_bst)
    if(mthd == "cb"){
      xxMb <- kMaxC(xx, r=r, k = 0)
    }
    theta_est <- gev_mle_cppPos(xxMb)$par
